...
class DualUR5EEGuiIK:
    def __init__(self,
        ...
        SACK_OBJ = resolve_existing_path([
            ...
            r"D:/Michael/2025/01.Research/01.Parceldetection/16.Pybullet/bullet3-master (1)/bullet3-master/examples/pybullet/examples/DeformableTest/object/sack.obj",
        ])
        ...
        # sack 기반 far 포인트 강제 오버라이드(버튼으로 on)
...
##############################20260302 sack 위위쪽
    ...
    def _update_sack_debug(self, force=False):
        ...
        # print(
        #     f"[SACK] center(x,y,z)=({c[0]:+.4f}, {c[1]:+.4f}, {c[2]:+.4f}) "
        #     f"rpy(rad)=({r:+.4f}, {pch:+.4f}, {y:+.4f}) "
        #     f"size(L,W,H)=({lx:.4f}, {wy:.4f}, {hz:.4f})"
        # )
    ...
    def _shape_points_local(self, geom_type, dims):
        # collision shape 기준 점 후보(로컬 프레임)
        ...
        if geom_type in (p.GEOM_CYLINDER, p.GEOM_CAPSULE):
            # dims: [radius, height, ...] (build별 차이 고려)
            r = float(dims[0]) if len(dims) > 0 else 0.0
    ...
    def _get_gripper_extreme_points(self, rid, joint6_idx, fallback_link_idx):
        ...
        # 1) 실제 그리퍼 링크 우선(plate_link), 없으면 기존 fallback 링크 사용
        preferred = self._find_link(rid, ["plate_link"])
        candidate_links = [preferred] if preferred >= 0 else [fallback_link_idx]
    ...
    def _update_robot_realtime_debug(self, force=False):
        ...
        for arm, rid, jids, ee_idx in arms:
            ...
            # print(f"[{arm} JOINT XYZ] " + " | ".join(joint_msgs))

            # gripper(=joint6 자손 링크)에서 joint6 기준 극점 3개 출력 + 파란 점
            joint6_idx = jids[-1]
            ...
            # print(
            #     f"[{arm} GRIPPER PTS] J6=({j6[0]:+.3f},{j6[1]:+.3f},{j6[2]:+.3f}) "
            #     f"far1=({far1[0]:+.3f},{far1[1]:+.3f},{far1[2]:+.3f}) "
            #     f"far2=({far2[0]:+.3f},{far2[1]:+.3f},{far2[2]:+.3f}) "
            #     f"near=({near_pt[0]:+.3f},{near_pt[1]:+.3f},{near_pt[2]:+.3f}) "
            #     f"yz_angle={yz_ang:.2f}deg[{tag}:{lo:.0f}~{hi:.0f}]"
            # )

            # far1/far2는 파랑, near는 시인성 위해 시안으로 표시
    ...
    def step(self):
        ...
        # sack pose/size 로깅 + 방향축 표시
        self._update_sack_debug()

        # 로봇 조인트/EE 포인트 디버그
        self._update_robot_realtime_debug()
