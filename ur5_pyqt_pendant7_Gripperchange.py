    ...
    def run(self):
        ...
        while self._running and p.isConnected():
            ...
            if self._cnt % 30 == 0:
                ...
                self.sig_state.emit({"qL": qL_now, "qR": qR_now, "eeL": pL+rL, "eeR": pR+rR})
...
class Pendant(QtWidgets.QWidget):
    ...
    def _populate_motion_table(self, arm):
        ...
        for r, rec in enumerate(motions):
            ...
            if q and len(q) >= 6:
                qtxt = f"{q[0]:+.2f}, {q[1]:+.2f}, {q[2]:+.2f}, {q[3]:+.2f}, {q[4]:+.2f}, {q[5]:+.2f}"
            else:
                qtxt = "-"
    ...
    def _stage_motion_from_table(self, arm, row):
        ...
        if q is not None and len(q) >= 6:
            q6 = np.array(q[:6], dtype=np.float32)
            ...
            if ee is None or len(ee) < 6:
                return
            ee6 = np.array(ee[:6], dtype=np.float32)
            self._write_ui_ee(arm, ee6)
            # IK로 q 만들고 stage
            if arm == "L":
    ...
    def _build_arm_panel(self, arm="L"):
        ...
        # ===== EE (spin + slider) =====
        ee0 = self.shared["L_ee"] if arm=="L" else self.shared["R_ee"]
        ...
        sR_sl, sR = self._make_slider_spin(a_lo, a_hi, float(ee0[3]), step=0.01, decimals=3)
        sP_sl, sP = self._make_slider_spin(a_lo, a_hi, float(ee0[4]), step=0.01, decimals=3)
        sY_sl, sY = self._make_slider_spin(a_lo, a_hi, float(ee0[5]), step=0.01, decimals=3)
        ...
        f1.addRow("Roll [rad]",  self._row_widget(sR, sR_sl))
        f1.addRow("Pitch [rad]", self._row_widget(sP, sP_sl))
        f1.addRow("Yaw [rad]",   self._row_widget(sY, sY_sl))
        ...
        for k, jid in enumerate(jids_probe):
            ...
            j_sl, j_sp = self._make_slider_spin(lo, hi, float(q0[k]), step=0.01, decimals=3)
        ...
        for i, (j_sp, j_sl) in enumerate(j_pairs):
            f2.addRow(f"J{i+1} [rad]", self._row_widget(j_sp, j_sl))
    ...
    def _read_ui_ee(self, arm):
        ...
        return np.array([sp.value() for sp in pack["ee"]], dtype=np.float32)

    def _read_ui_q(self, arm):
        pack = self.left if arm=="L" else self.right
        return np.array([sp.value() for sp in pack["j"]], dtype=np.float32)

    def _write_ui_q(self, arm, q6):
        pack = self.left if arm=="L" else self.right
        for sp, v in zip(pack["j"], q6):
            self._set_pair_value(sp, v)
    
    def _write_ui_ee(self, arm, ee6):
        pack = self.left if arm=="L" else self.right
        for sp, v in zip(pack["ee"], ee6):
            self._set_pair_value(sp, v)
    ...
    def _on_state(self, st: dict):
        ...
        self.left["lab_ee"].setText(f"EE(actual): x={eeL[0]:.3f}, y={eeL[1]:.3f}, z={eeL[2]:.3f}, rpy=({eeL[3]:+.2f},{eeL[4]:+.2f},{eeL[5]:+.2f})")
        self.left["lab_q"].setText("Q(actual): " + " ".join([f"{x:+.2f}" for x in qL]))

        self.right["lab_ee"].setText(f"EE(actual): x={eeR[0]:.3f}, y={eeR[1]:.3f}, z={eeR[2]:.3f}, rpy=({eeR[3]:+.2f},{eeR[4]:+.2f},{eeR[5]:+.2f})")
        self.right["lab_q"].setText("Q(actual): " + " ".join([f"{x:+.2f}" for x in qR]))
    ...
    def _save_motion(self, arm: str):
        # 1) 펜던트(타겟) 값 읽기
        ee_t = self._read_ui_ee(arm).tolist()   # [x,y,z,r,p,y]
        q_t  = self._read_ui_q(arm).tolist()    # [j1..j6]
    
        # 2) 시뮬(실제) 값 읽기
        if arm == "L":
            q_a = [float(p.getJointState(self.sim.urL, jid)[0]) for jid in self.sim.jL]
            (pos, rpy) = self.sim._get_ee_pose(self.sim.urL, self.sim.eeL)
            ee_a = [float(x) for x in (pos + rpy)]
            out_path = self.log_left
            lab = self.left.get("lab_save", None)
        else:
            q_a = [float(p.getJointState(self.sim.urR, jid)[0]) for jid in self.sim.jR]
